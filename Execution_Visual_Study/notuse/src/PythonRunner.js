...
exports.runPythonInstrumentation = runPythonInstrumentation;
exports.runSuspiciousVisualizationPipeline = runSuspiciousVisualizationPipeline;
...
function isDebugPromptText(text) {
...
async function runPythonInstrumentation(context) {
...
async function runSuspiciousVisualizationPipeline(context) {
    ...
    const panel = vscode.window.createWebviewPanel('visualPilotSuspiciousWorkflow', 'VisualPilot Suspicious Visualization', vscode.ViewColumn.One, {
        enableScripts: true,
...
async function runPythonInstrumentationWithArgs(context, args) {
    const channel = ensureOutputChannel();
    channel.show();
    ...
    const pythonPath = await (0, PythonEnv_1.resolvePythonPath)(channel);
...
function validatePythonInstConfig(instJsonPath) {
...
function buildPipelinePaths(context, workspaceRoot) {
...
function buildInitialVisualizeFormData(paths) {
    const defaults = {
    ...
    if (fs.existsSync(paths.instJsonPath)) {
        try {
            const existing = JSON.parse(fs.readFileSync(paths.instJsonPath, 'utf-8'));
...
function readAnimeDefaults(animeJsonPath) {
    ...
    try {
        const parsed = JSON.parse(fs.readFileSync(animeJsonPath, 'utf-8'));
        return {
...
function getPipelineSettingsFromForm(data) {
...
function saveInstConfig(instJsonPath, workspaceRoot, data) {
...
function saveAnimeConfig(animeJsonPath, data) {
...
async function runStep0(paths, channel) {
    const pythonPath = await (0, PythonEnv_1.resolvePythonPath)(channel);
...
async function runStep1(paths, channel, settings, animeJsonPath) {
    const pythonPath = await (0, PythonEnv_1.resolvePythonPath)(channel);
...
async function runStep2(paths, channel, runDir) {
    const pythonPath = await (0, PythonEnv_1.resolvePythonPath)(channel);
...
async function runStep3(paths, channel, settings, animeJsonPath, runDir) {
    const pythonPath = await (0, PythonEnv_1.resolvePythonPath)(channel);
    const targetAnime = animeJsonPath || paths.animeJsonPath;
...
async function runStep4(channel, runDir) {
    const pythonPath = await (0, PythonEnv_1.resolvePythonPath)(channel);
...
function findGeneratedVideoPath(resultDir) {
...
function resolveActiveRunDir(designWorkDir, latestRunDir) {
...
function getVisualizeWorkflowWebviewHtml(initialData) {
...
function getLatestRunDir(designWorkDir) {
...
function buildBlockSegmentArgs(blocksEntirePath) {
    if (!fs.existsSync(blocksEntirePath)) {
        throw new Error(`blocks_entire.json not found: ${blocksEntirePath}`);
    }
    const parsed = JSON.parse(fs.readFileSync(blocksEntirePath, 'utf-8'));
...
async function runPythonProcess(input) {
    ...
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)(input.pythonPath, [input.scriptPath, ...input.args], {
            cwd: input.cwd,
...
async function runCommandProcess(input) {
    ...
    return new Promise((resolve) => {
        const proc = (0, child_process_1.spawn)(input.command, input.args, {
            cwd: input.cwd,
...
async function handleDebugInteraction(title, workDir) {
    ...
    vscode.window.showInformationMessage(`${title}: Edit response.txt and save. The pipeline will continue automatically once response.txt is non-empty.`);
...
async function openTextFileIfExists(filePath) {
