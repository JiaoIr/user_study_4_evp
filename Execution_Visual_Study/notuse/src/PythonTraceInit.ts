...
export async function initPythonTrace(context: vscode.ExtensionContext) {
    ...
    const panel = vscode.window.createWebviewPanel(
        'visualPilotPythonInstSettings',
        'VisualPilot Python Trace Settings',
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        switch (message.command) {
            ...
            case 'saveAndRun': {
                try {
                    const parsed = JSON.parse(message.data as string) as { json?: Record<string, unknown> };
                    const jsonData = parsed.json as Record<string, unknown> | undefined;
...
function getPythonWebviewContent(data: Record<string, unknown>): string {
