...
class TraceViewProvider {
    extensionUri;
    getActiveCaseRoot;
    getActiveSourceCaseRoot;
    static viewType = 'ttdTraceView';
    _view;
    workspaceRoot;
    traceData = [];
    blockData = [];
    decorationType;
    playbackDecorationTypes = [];
    playbackFilePath;
    fileWatcher;
    constructor(extensionUri, getActiveCaseRoot, getActiveSourceCaseRoot) {
        this.extensionUri = extensionUri;
        this.getActiveCaseRoot = getActiveCaseRoot;
        this.getActiveSourceCaseRoot = getActiveSourceCaseRoot;
    ...
    setupFileWatcher() {
        ...
        this.fileWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(this.workspaceRoot, '**/__debug/**/trace.json'));
    ...
    async highlightPlaybackBlock(block) {
    ...
    async showAllPlaybackBlocks(blocks) {
        const mapped = blocks
        ...
        await this.highlightCodeRanges(blocks.map((block) => ({
            block,
            colorIndex: ((block.id % PLAYBACK_BLOCK_COLORS.length) + PLAYBACK_BLOCK_COLORS.length) % PLAYBACK_BLOCK_COLORS.length
        })));
    }
    ...
    async highlightCodeRanges(entries) {
        const validEntries = entries.filter(({ block }) => !!block.file &&
        ...
        const filePath = this.resolveSourceFilePath(validEntries[0].block.file);
        ...
        for (let idx = 0; idx < this.playbackDecorationTypes.length; idx += 1) {
            ...
            for (const entry of validEntries) {
                ...
                const startLine = Math.max(0, entry.block.cstart - 1);
                const endLine = Math.max(startLine, entry.block.cend - 1);
        ...
        const revealRange = new vscode.Range(Math.max(0, revealEntry.cstart - 1), 0, Math.max(0, revealEntry.cend - 1), 0);
    ...
    resolveSourceFilePath(fileValue) {
...
//# sourceMappingURL=TraceViewProvider.js.map