...
export class SlidesOverlayController implements vscode.Disposable {
    private panel?: vscode.WebviewPanel;
    private outputChannel: vscode.OutputChannel;
    private readonly disposables: vscode.Disposable[] = [];
    private linkageBlocks: AnimeBlockSegment[] = [];

    constructor(
        private readonly context: vscode.ExtensionContext,
        private readonly extensionUri: vscode.Uri,
        private readonly onPlaybackBlockChanged?: (block?: AnimeBlockSegment) => void,
        private readonly onShowAllPlaybackBlocks?: (blocks: AnimeBlockSegment[]) => void,
    ...
    public async terminate(showMessage = true, syncAllBlocks = true): Promise<void> {
    ...
    private ensurePanel(workspaceRoot: string): void {
        ...
        this.panel = vscode.window.createWebviewPanel(
            'visualPilotSlidesOverlay',
            'Execution_Visual_Study Slides',
            {
        ...
        const messageDisposable = this.panel.webview.onDidReceiveMessage(async (message: { command?: string; blockId?: number | null }) => {
    ...
    private async refreshVideoSource(workspaceRoot: string, notifyWhenMissing: boolean): Promise<void> {
        if (!this.panel) {
            return;
        }
    ...
    private loadBlocks(workspaceRoot: string): AnimeBlockSegment[] {
        ...
        try {
            ...
            const rawBlocks: any[] = parsed.blocks;
            return rawBlocks
                .filter((item: any) =>
                ...
                .map((item: any): AnimeBlockSegment => ({
                    id: item.id,
                    name: item.name,
                    start: item.start,
                    end: item.end,
                ...
                .sort((a: AnimeBlockSegment, b: AnimeBlockSegment) => a.id - b.id);
        } catch {
            return [];
        }
    }

    private findTargetVideo(workspaceRoot: string): string | undefined {
        const visualPilotRoot = path.join(workspaceRoot, '__debug', 'visualpilot');
        const videoPath = path.join(visualPilotRoot, TARGET_SCENE_VIDEO);
        return fs.existsSync(videoPath) ? videoPath : undefined;
    }

    private postControl(command: 'play' | 'terminate'): void {
        if (!this.panel) {
            return;
        }
