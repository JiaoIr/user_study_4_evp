...
import { PlaybackLinkBlock, TraceViewProvider } from './TraceViewProvider';
import { SlidesOverlayController } from './SlidesOverlayController';
import { LlmGenerationController } from './LlmGenerationController';
import { PythonTutorController } from './PythonTutorController';

let currentId: string | undefined;

interface MainAnswerRecord {
	case_id: string;
	question: string;
	answer: string;
	single_start: string;
	single_end: string;
	query_answer?: Record<string, Record<string, { type: string; content: string; response: string }>>;
	debugging_start?: string;
	debugging_end?: string;
	debugging_query_answer?: Record<string, Record<string, { type: string; content: string; response: string }>>;
	understanding_start?: string;
	understanding_end?: string;
	understanding_query_answer?: Record<string, Record<string, { type: string; content: string; response: string }>>;
}

interface MainResultRecord {
	id: string;
	main_start: string;
	main_end: string;
	answer: MainAnswerRecord[];
	rtlx?: Array<{
		group: string;
		scores: {
			mentalDemand: number;
...
interface QueryQuestionItem {
	type: string;
	content: string;
}

type QueryTemplate = Record<string, Record<string, QueryQuestionItem>>;
type AnswerPhase = 'debugging' | 'understanding';

function toTimestampForFile(d: Date): string {
...
function getExplainCommandByType(explainType: string): string {
...
function toGroupLabel(explainType: string): string {
...
function loadStudyQuestionsFromConfig(extensionRoot: string, participantId: string): StudyQuestion[] | undefined {
	...
	let parsed: Record<string, StudyConfigEntry[]>;
	try {
		parsed = JSON.parse(fs.readFileSync(configPath, 'utf8')) as Record<string, StudyConfigEntry[]>;
	} catch {
		return undefined;
	}
...
function loadQueryTemplate(caseRoot: string): QueryTemplate {
	...
	try {
		const parsed = JSON.parse(fs.readFileSync(queryPath, 'utf8')) as Record<string, Record<string, QueryQuestionItem>>;
...
function pickQuerySectionByPhase(query: QueryTemplate, phase: AnswerPhase): QueryTemplate {
...
function listAvailableCaseIds(workspaceRoot: string): string[] {
...
function getActiveWorkspaceCaseRoot(): string | undefined {
...
function getActiveDebugCaseRoot(extensionRoot: string): string | undefined {
	if (!currentId) {
		return undefined;
	}
...
function collectPythonFiles(rootDir: string): string[] {
...
async function focusCaseSource(caseRoot: string): Promise<void> {
...
function getMainStudyHtml(webview: vscode.Webview): string {
...
function getRtlxHtml(webview: vscode.Webview, groupType: string): string {
	const nonce = String(Date.now());
	const csp = `default-src 'none'; style-src ${webview.cspSource} 'unsafe-inline'; script-src 'nonce-${nonce}';`;
...
export function activate(context: vscode.ExtensionContext) {
	let mainStudyPanel: vscode.WebviewPanel | undefined;
	let rtlxPanel: vscode.WebviewPanel | undefined;
	let participantId = '';
	let loadedQuestionBank = false;
	let isAdminMode = false;
	let orderedQuestions: StudyQuestion[] = [];
	let nextCaseIndex = 0;
	let pendingRtlxGroupType: string | undefined;
	let rtlxCompletedGroups = new Set<string>();
	let activeAnswerCaseId: string | undefined;
	let activeExplainType: string | undefined;
	let activeAnswerRecordIndex: number | undefined;
	let activeAnswerPhase: AnswerPhase | undefined;
	let activeQueryTemplate: QueryTemplate | undefined;
	let mainResultFilePath: string | undefined;
	let mainResultRecord: MainResultRecord | undefined;
	...
	const resetMainStudyState = () => {
		...
		rtlxCompletedGroups = new Set<string>();
	...
	const traceViewProvider = new TraceViewProvider(
		context.extensionUri,
		() => getActiveDebugCaseRoot(context.extensionUri.fsPath),
		() => getActiveWorkspaceCaseRoot()
	);
	const slidesOverlayController = new SlidesOverlayController(
		context,
		context.extensionUri,
		async (block) => {
			if (!block) {
			...
			await traceViewProvider.highlightPlaybackBlock(block as PlaybackLinkBlock);
		},
		async (blocks) => {
			if (!blocks.length) {
				traceViewProvider.clearPlaybackHighlight();
			...
			await traceViewProvider.showAllPlaybackBlocks(blocks as PlaybackLinkBlock[]);
		},
		() => getActiveDebugCaseRoot(context.extensionUri.fsPath)
	);
	const llmGenerationController = new LlmGenerationController(
		context.extensionUri,
		() => getActiveDebugCaseRoot(context.extensionUri.fsPath)
	);
	const pythonTutorController = new PythonTutorController(
		context.extensionUri,
		async (block) => {
			if (!block) {
				traceViewProvider.clearPlaybackHighlight();
			...
			const traceStart = typeof (block as { tstart?: number }).tstart === 'number'
				? (block as { tstart?: number }).tstart
				: block.id;
			const traceEnd = typeof (block as { tend?: number }).tend === 'number'
				? (block as { tend?: number }).tend
				: traceStart;
			...
			await traceViewProvider.highlightPlaybackBlock({
				...
				tstart: traceStart,
				tend: traceEnd,
				cstart: (block as { cstart?: number }).cstart,
				cend: (block as { cend?: number }).cend,
				file: block.file
			});
		},
		() => getActiveDebugCaseRoot(context.extensionUri.fsPath)
	...
	const showMainStudyCommand = vscode.commands.registerCommand('Execution_Visual_Study.showMainStudy', async () => {
		...
		mainStudyPanel = vscode.window.createWebviewPanel(
			'Execution_Visual_StudyMainStudy',
			'Main Study',
		...
		const onMessage = mainStudyPanel.webview.onDidReceiveMessage(async (message: { command?: string; caseId?: string; participantId?: string; queueIndex?: number; queryAnswer?: unknown; phase?: string }) => {
			...
			if (message.command === 'submit') {
				...
				const queryAnswer = message.queryAnswer && typeof message.queryAnswer === 'object'
					? (message.queryAnswer as Record<string, Record<string, { type: string; content: string; response: string }>>)
					: {};
				const phase: AnswerPhase = activeAnswerPhase === 'understanding' ? 'understanding' : 'debugging';
				const now = toIsoTime(new Date());
			...
			if (message.command === 'openRtlx') {
				const groupType = String((message as { groupType?: string }).groupType ?? '').trim();
				if (!groupType) {
					return;
				}
				...
				rtlxPanel = vscode.window.createWebviewPanel(
					'Execution_Visual_StudyRtlx',
					'阶段自评：工具使用感受',
					vscode.ViewColumn.Beside,
					{
						enableScripts: true,
				...
				const rtlxMessage = rtlxPanel.webview.onDidReceiveMessage((rtlxMessageData: { command?: string; group?: string; scores?: any; feedback?: string }) => {
	...
	context.subscriptions.push(
		refreshTraceCommand,
		openTraceViewerCommand,
		explainByVisualPilotCommand,
		explainByPythonTutorCommand,
		explainByLlmGenerationCommand,
		showMainStudyCommand,
		terminateSlidesCommand,
		slidesOverlayController,
		llmGenerationController,
		pythonTutorController
	);
}
