...
export class PythonTutorController implements vscode.Disposable {
    private panel?: vscode.WebviewPanel;
    private readonly disposables: vscode.Disposable[] = [];
    private steps: PythonTutorStep[] = [];

    constructor(
        private readonly extensionUri: vscode.Uri,
        private readonly onStepChanged?: (block?: PythonTutorBlock) => void,
    ...
    private ensurePanel(workspaceRoot: string): void {
        ...
        const messageDisposable = this.panel.webview.onDidReceiveMessage(async (message: { command?: string; blockId?: number }) => {
    ...
    private loadBlocks(blocksPath: string): PythonTutorBlock[] {
        if (!fs.existsSync(blocksPath)) {
            return [];
        }

        try {
            const raw = JSON.parse(fs.readFileSync(blocksPath, 'utf8')) as { file?: string; blocks?: any[] };
    ...
    private loadImages(graphRoot: string, webview: vscode.Webview): Array<{ id: number; src: string }> {
        ...
        const images = entries
            ...
            .filter((item): item is { id: number; src: string } => !!item)
            .sort((a, b) => a.id - b.id);
